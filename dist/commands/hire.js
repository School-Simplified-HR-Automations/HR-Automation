...
module.exports = {
    ...
    execute(interaction) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const sw = new stopwatch_1.Stopwatch(2).start();
            let team = interaction.options.getString("team");
            const posres = yield __1.dbSql.query(`SELECT * FROM positions WHERE title = '${interaction.options.getString("position")}'`, { type: sequelize_1.QueryTypes.SELECT });
            if (posres.length < 1) {
                return interaction.editReply({
                    embeds: [
                        new discord_js_1.EmbedBuilder()
                            .setTitle("Error During Hire")
                            .setDescription("The position you requested does not exist in our system. Please try again, being mindful of capitalization and punctuation if necessary."),
                    ],
                });
            }
            const deptres = yield __1.dbSql.query(`SELECT * FROM departments WHERE name = '${interaction.options.getString("department")}'`, { type: sequelize_1.QueryTypes.SELECT });
            if (deptres.length < 1) {
                return interaction.editReply({
                    embeds: [
                        new discord_js_1.EmbedBuilder()
                            .setTitle("Error During Hire")
                            .setDescription("The department you requested does not exist in our system. Please try again, being mindful of capitalization and punctuation if necessary."),
                    ],
                });
            }
            let teamres = [];
            if (team) {
                teamres = yield __1.dbSql.query(`SELECT * FROM teams WHERE name = '${team}'`, { type: sequelize_1.QueryTypes.SELECT });
                if (teamres.length < 1) {
                    return interaction.editReply({
                        embeds: [
                            new discord_js_1.EmbedBuilder()
                                .setTitle("Error During Hire")
                                .setDescription("The team you requested does not exist in our system. Please try again, being mindful of capitalization and punctuation if necessary."),
                        ],
                    });
                }
            }
                    ...
                    return interaction.editReply({
                        embeds: [
                            new discord_js_1.EmbedBuilder()
                                ...
                                .setDescription(`There was an unexpected error during raw querying:\n\n${err}`),
                        ],
                    });
                });
                const tpt = nodemailer_1.default.createTransport({
                    service: "gmail",
                    auth: {
  ...
  <i>Hiring Team</i>
                ...
                if (did) {
                    yield (yield interaction.client.users.fetch(`${did}`)).send({
                        content: "Hello there! Thanks for your application to our team at School Simplified. Please check the personal email you listed on your application for a status update. Thanks!\n\nRegards,\nHuman Resources",
                    });
                }
                return interaction.editReply({
                    embeds: [
                        new discord_js_1.EmbedBuilder()
                            .setTitle("Done!")
                            .setDescription(`The user was successfully hired in ${sw.stop().toString()}`)
                            .setColor("Green"),
                    ],
                });
            }
