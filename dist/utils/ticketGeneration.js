...
function ticketGeneration(text, author, category = "geninq", priority = 0, self) {
    const frame = "------------------------------------------";
    let ctgText = "";
    switch (category) {
        case "bot":
    ...
    if (priority && priority > 3)
        subject = `URGENT: ${ctgText} - ${author}`;
    else
        subject = `${ctgText} - ${author}`;
    return {
...
exports.default = ticketGeneration;
