...
export default class StaffFileQueryRoutes {
    ...
    async getTeamByStaffTeam(staff: object): Promise<object> {
        return dbSql.query(`SELECT * FROM teams WHERE id = (SELECT TeamId FROM stafffiles WHERE id = ${(staff as StaffFile).id})`) // TODO
    }
    /**
    ...
    async getDepartmentByStaffDepartment(staff: object): Promise<object> {
        return dbSql.query(`SELECT * FROM departments WHERE id = (SELECT DepartmentId FROM stafffiles WHERE id = ${(staff as StaffFile).id})`)
    }
    /**
